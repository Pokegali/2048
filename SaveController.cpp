...
void SaveController::loadData()
{
    ...
    if(size != 0)
    {
        auto bestScoreIterator= std::max_element(this->scores.begin(), this->scores.end(),
                                                 [](const ScoreRecord& record1, const ScoreRecord& record2)
...
{
    if(record.score < this->bestScore.score)
    {
        return;
    }
